...
<%
    ...
    List<Homework> list = UserDao.GethomeworkByClass(Class_ID,Subject_ID);
...
<div class="container">
    ...
    <%
        if(list.isEmpty()){
            out.print("<h1 style=\"margin-top: 300px;\">დავალებები არ არის</h1>");
        }else{
    %>

      <div class="wrapper">
          <c:forEach items="${list}" var="u">
              <div class="davaleba-div">
                  <p class="Davaleba" name="davaleba" id="davaleba">${u.getHomework()}</p>
                  <label for="deadLine" class="deadline-lbl">შესრულების ვადა</label> <br>
                  <p class="deadline" id="deadLine">${u.getDeadline()}</p>
                  <label for="Startdate" class="startdate-lbl">მიღების თარიღი</label> <br>
                  <p class="startDate" id="Startdate">${u.getHomeworkDate()}</p>
                  <a id="redaqtireba" type="button" class="btn btn-success" href="formEdit.jsp?Homework=${u.getHomework()}&startDate=${u.getHomeworkDate()}&Deadline=${u.getDeadline()}&Class_ID=${u.getClass_ID()}&Class_Name=${u.getClass_Name()}&Subject_ID=${u.getSubject_ID()}&Homework_ID=${u.getHomework_ID()}">Edit</a>
                  <a href="viewWhohasdone.jsp?Class_ID=${u.getClass_ID()}&Class_Name=${u.getClass_Name()}&Homework_ID=${u.getHomework_ID()}" class="btn btn-primary" >ნახვა</a>
                  <a href="../TeacherProcess/deleteHomework.jsp?Class_ID=${u.getClass_ID()}&Class_Name=${u.getClass_Name()}&Homework_ID=${u.getHomework_ID()}&Subject_ID=${Subject_ID}" class="btn btn-danger">წაშლა</a>
              </div>
          </c:forEach>
